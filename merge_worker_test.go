...
func TestMergeWorker_ContextCancellation(t *testing.T) {
	...
	// Cancel the merge worker's context (simulating shutdown signal)
	db.mergeWorker.cancel()

	// Try to trigger merge after cancellation - should fail immediately
	err = db.mergeWorker.TriggerMerge()
