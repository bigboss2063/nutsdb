...
func (db *DB) NewWriteBatch() (*WriteBatch, error) {
	...
	tx, err := db.transactionManager.BeginTx(true, false)
	if err != nil {
		return nil, err
	}
	wb.tx = tx
...
func (wb *WriteBatch) Cancel() error {
	...
	if wb.tx != nil {
		wb.db.transactionManager.UnregisterTx(wb.txID)
...
func (wb *WriteBatch) commit() error {
	...
	// needLock=false because Put()/Delete() will acquire lock before operations
	var err error
	wb.tx, err = wb.db.transactionManager.BeginTx(true, false)
	if err != nil {
		return err
	}
	wb.txID = wb.tx.id
	...
	wb.db.transactionManager.UnregisterTx(committingTxID)
